...
- **Pendente (30/06) — cartão exportado ainda sai com "abas laterais".** Montgomery reportou que a imagem exportada/impressa ainda sai com uma margem/faixa lateral em volta do cartão (em vez do cartão preencher a imagem inteira, borda a borda). Ainda não investigado a fundo — provável candidato: a `box-shadow`/padding do container `.fc` ao redor de `#rxPrevC`, ou margem do próprio `html2canvas` capturando além do elemento de 420px. Conferir com calma na próxima sessão antes de alterar.
