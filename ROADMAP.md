...
- **Recomendação Terapêutica — visual do card/print ainda "não está bacana" (29/06):** Montgomery testou após o ajuste do texto curto de WhatsApp e achou que o cartão impresso/visualizado (Visualizar, Imprimir, Exportar JPG) ainda não ficou do jeito que ele quer — não detalhou o quê exatamente (pode ser estético, pode ser ainda estar com cache antigo no celular/navegador, já que ele mesmo registrou a dúvida "pode ser que ainda não atualizou completamente"). O recurso de "reaproveitar de outro paciente como modelo" ele confirmou que funcionou. **Próximo passo:** perguntar a ele especificamente o que está errado no visual (cores, espaçamento, informação faltando/sobrando) antes de tentar corrigir, e confirmar se é mesmo o código mais recente que está sendo exibido (pedir pra forçar atualização da página/PWA) antes de assumir que é um bug novo.
